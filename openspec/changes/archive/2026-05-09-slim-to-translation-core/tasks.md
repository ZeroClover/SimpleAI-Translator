...
- [x] 6.11 保留并完善 TTS provider / voice / volume / rate 设置 UI
- [x] 6.12 在 TTS 设置区新增 OpenAI TTS 子区:provider 引用下拉(仅显示 `openai-chat` / `openai-responses`,Anthropic 条目隐藏或禁用)、模型 Combobox(经 `filterTTSModels` 白名单过滤,空时引导手填)、voice 下拉(内置 `alloy / ash / ballad / coral / echo / fable / onyx / nova / sage / shimmer / verse / marin / cedar` + 自定义输入/voice id)、音频格式下拉(默认 mp3,含 pcm)
...
- [x] 10.6 在桌面端运行 `pnpm dev-tauri` 手测同上 + 主热键 + 显示窗口热键
...
- [x] 12.4 在干净的浏览器 profile 与全新 Tauri 配置下重启,验证首次启动引导(无 Provider 时禁用翻译并提示)
- [x] 12.5 用一份带旧 settings 的浏览器 profile 启动,验证旧 OpenAI / Azure / 未识别 Provider 字段不会产出 ProviderConfig,保存后不再写回旧字段
- [x] 12.6 配置一份 OpenAI Provider,验证 Provider 表单的"刷新模型"下拉显示已过滤的对话模型,Combobox 手填值仍可生效
- [x] 12.7 把 TTS 切到 OpenAI,验证模型 Combobox 仅显示 `tts-1` / `tts-1-hd` / `gpt-4o-mini-tts` / `gpt-4o-mini-tts-YYYY-MM-DD`,选定后朗读成功
- [x] 12.8 删除被 OpenAI TTS 引用的 Provider,验证 TTS 自动回退到 Edge 并 toast 提示
- [x] 12.9 在浏览器扩展中配置自定义 endpoint,验证 optional host permission 授权通过后可刷新模型,拒绝授权时不会发起请求
